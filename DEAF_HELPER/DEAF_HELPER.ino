...
void setup() {
  ...
  // MP3 (DFPlayer Mini) init – SAME STYLE AS WORKING EXAMPLE
  Serial.println("Starting MP3 UART...");
  MP3Serial.begin(9600, SERIAL_8N1, MP3_RX_PIN, MP3_TX_PIN);
  delay(1000);   // give DFPlayer time to boot

  Serial.println("Initializing DFPlayer (with ACK + reset) ...");
  if (!dfPlayer.begin(MP3Serial, /*isACK =*/ true, /*doReset =*/ true)) {
    Serial.println(F("DFPlayer init FAILED"));
    Serial.println(F("1) Check RX/TX wiring (ESP32 16=RX1 <- DF TX, 17=TX1 -> DF RX)"));
    Serial.println(F("2) Check 5V, GND and speaker"));
    Serial.println(F("3) Check SD card + files"));
    ...
    Serial.println(F("DFPlayer volume set to 25"));
  }
...
/* ==================================================== */
void loop() {
  // Handle DFPlayer status / errors (like example)
  if (dfPlayer.available()) {
    uint8_t type  = dfPlayer.readType();
    int value     = dfPlayer.read();
    printDFPDetail(type, value);
  }
...
void playNameAudio() {
  ...
  dfPlayer.play(TRACK_NAME);   // Track 2 on SD card
  Serial.println("[DFP] Playing NO audio (track 2)");
  delay(2000);
  dfPlayer.stop();
}

/* ========= DFPLAYER DEBUG (LIKE EXAMPLE) ========= */

void printDFPDetail(uint8_t type, int value) {
  switch (type) {
    case TimeOut:
      Serial.println(F("[DFP] Time Out!"));
      break;
    case WrongStack:
    ...
    case DFPlayerPlayFinished:
      ...
      break;
    case DFPlayerError:
      Serial.print(F("[DFP] DFPlayerError: "));
      switch (value) {
        case Busy:
