...
const uint8_t bhkTypes[NUM_ROOMS_PER_FLOOR] = {
  1, 1, 3, 3, 1, 1  // Example mapping: Rooms 0-1 are 1BHK, 2-3 are 2BHK, 4-7 are 3BHK
};
...
void setup() {
  ...
  for (int floor = 0; floor < NUM_FLOORS; floor++) {
    ...
    if (dataPin != -1) {
      ...
      ledsFloor[floor]->setBrightness(150);
  ...
  for (int floor = 0; floor < NUM_FLOORS; floor++) {
    for (int i = 0; i < TOTAL_LEDS_PER_FLOOR; i++) {
      ledsFloor[floor]->setPixelColor(i, ledsFloor[floor]->Color(255, 222, 50));
    }
...
void runOneByOnePattern() {
  ...
  for (int floor = 0; floor < NUM_FLOORS; floor++) {
    for (int led = 0; led < TOTAL_LEDS_PER_FLOOR; led++) {
      ledsFloor[floor]->setPixelColor(led, ledsFloor[floor]->Color(255, 150, 255)); // Blue
    }
    ledsFloor[floor]->show();
    delay(100);
  }
