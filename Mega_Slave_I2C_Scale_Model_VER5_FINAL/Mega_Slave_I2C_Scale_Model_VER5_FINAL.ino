...
void runOneByOnePattern() {
  ...
  for (int floor = 0; floor < NUM_FLOORS; floor++) {
    for (int led = 0; led < TOTAL_LEDS_PER_FLOOR; led++) {
      ledsFloor[floor]->setPixelColor(led, ledsFloor[floor]->Color(0, 0, 255)); // Blue
      ledsFloor[floor]->show();
      delay(10);
    }
    delay(100);
...
// Pattern 2: Floor Wipe
void runFloorWipePattern() {
  Serial.println("Running Pattern: Floor Wipe");
  for (int floor = 0; floor < NUM_FLOORS; floor++) {
    for (int led = 0; led < TOTAL_LEDS_PER_FLOOR; led++) {
      ledsFloor[floor]->setPixelColor(led, ledsFloor[floor]->Color(0, 255, 0)); // Green
    }
    ledsFloor[floor]->show();
    delay(100);
...
void runFloorAlternatePattern() {
  ...
  for (int cycle = 0; cycle < 5; cycle++) {
    for (int floor = 0; floor < NUM_FLOORS; floor++) {
      uint32_t color = (floor % 2 == 0) ? ledsFloor[floor]->Color(255, 0, 0) : ledsFloor[floor]->Color(0, 0, 255);
      for (int led = 0; led < TOTAL_LEDS_PER_FLOOR; led++) {
