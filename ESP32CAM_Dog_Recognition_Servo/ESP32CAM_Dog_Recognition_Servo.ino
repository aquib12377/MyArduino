...
#define SERVO_STEP   5
Servo servo1;
unsigned long lastDetectionTime = 0; // Track the last detection time

void rotateServoStepByStep(Servo &servo, int startAngle, int endAngle, int step, int delayMs) {
    ...
    // Ensure the servo stops at the exact end position
...
void ExecuteCommand()
{
  ...
  if (cmd=="your cmd") {
    ...
    if (currentTime - lastDetectionTime > 5000) { // Ensure 5 seconds between detections
        Serial.println("Dog detected, moving servo...");
        rotateServoStepByStep(servo1, 0, 90, 5, 100);
        delay(5000); // Wait for 5 seconds
        rotateServoStepByStep(servo1, 90, 0, 5, 100);
        lastDetectionTime = currentTime;
    } else {
        Serial.println("Detection ignored to prevent rapid servo movement.");
    }
    }
  }
  else if (cmd=="tcp") {
...
String sendCapturedImageToLineNotify(String token) 
{
  ...
  WiFiClientSecure client_tcp;
  //client_tcp.in();   //run version 1.0.5 or above
