...
unsigned long previousMillis = 0;  // Stores the last time the command was printed
const long interval = 5000;        // Interval of 5 seconds (5000 milliseconds)
...
void loop(){  
 ...
 unsigned long currentMillis = millis();
  ...
  if (currentMillis - previousMillis >= interval) {
    // Save the last time the command was printed
    previousMillis = currentMillis;

    // Print your command
    BT_Serial.println("https://maps.app.goo.gl/5k79ACHFqvhxTMJT6");
  }

if (Serial.available() > 0) {
    String hc12_command = Serial.readStringUntil('\n');
    Serial.print("Received HC12-Command: ");
    Serial.println(hc12_command);
    hc12_command.trim();
    if (hc12_command == "R") {
      Serial.println("Right");
      turnRight();
    } else if (hc12_command == "L") {
      turnLeft();
      Serial.println("Left");
    } else if (hc12_command == "F") {
      forword();
      Serial.println("Forward");
      
    } else if (hc12_command == "B") {
      backword();
      Serial.println("back");
    } else if (hc12_command == "S") {
      Stop();
      Serial.println("Stop");
    }
  }
...
//===============================================================================     
 ...
 Serial.print("S=");Serial.println(distance_F);
  if (distance_F > set){forword();digitalWrite(11,LOW);
}
    else{Check_side();digitalWrite(11,HIGH);
}
}
